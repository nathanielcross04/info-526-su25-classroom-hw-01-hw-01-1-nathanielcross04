...
county |>
  ggplot(aes(x = homeownership, y = poverty)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "green") + # NOTE: I could not figure out which smoothing/regression model was utilized in the original plot and I tried all the method options (and other function options), so I used OLS as a replacement.
...
credit |>
  ...
  geom_point() +
  facet_grid(rows = vars(student), cols = vars(married), labeller(label_parsed())) +
  labs(
    x = "Income",
  ...
  theme(legend.position= "none") # Source: Andrew 2021 (StackOverflow)
